    ...
    import xin.cosmos.common.basic.dict.business.BillAcceptanceBusiType;
    ...
    public class BillAcceptanceDisclosureService {
         ...
         */
        @SneakyThrows
        public void upload(MultipartFile file, String busiType) {
            // 业务类型
         ...
         * @return
         */
        public List<BillAcceptanceVO> queryCorpBillAcceptance(String date, String busiType) {
            if (StringUtils.isEmpty(date)) {
                throw new RuntimeException("披露信息时点日期不能为空");
            }
            // 业务类型
            BillAcceptanceBusiType acceptanceBusiType = IDict.findByName(busiType, BillAcceptanceBusiType.class);
            if (acceptanceBusiType == null) {
                throw new RuntimeException("文件下载类型为空");
            }
            String showMonth = (date.length() > 7) ? date.substring(0, 7) : date;
            final AtomicInteger index = new AtomicInteger(0);
            List<CorpEntity> corpEntities = redisService.getList(Constant.getMetaDataStoreKey(acceptanceBusiType.name()));
